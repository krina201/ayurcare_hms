document.addEventListener('DOMContentLoaded', function () {
                    ...
                    <div class="flex items-center">
                        <div class="flex-shrink-0 h-8 w-8 mr-3">
                            <img class="h-8 w-8 rounded-full object-cover" 
                                 src="${safePhotoPath ? safePhotoPath : '/backend-assets/media/uploads/products/patient_20250825104721_ggatR2.jpg'}" 
                                 alt="Patient avatar"
                                 onerror="this.src='/backend-assets/media/uploads/products/patient_20250825104721_ggatR2.jpg'">
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="text-sm font-medium text-gray-900 truncate">${safeName}</div>
    ...
    function updatePatientCard(uhid, name, gender, age, mobile, prakriti, allergies, photoPath) {
        ...
        const patientPhotoPath = photoPath || '/backend-assets/media/uploads/products/patient_20250825104721_ggatR2.jpg';
    ...
    function updatePrescriptionMedicines(items) {
        ...
        items.forEach((item, index) => {
            const medicineId = `medication_${index + 1}`;
            selectedMedicines.push({
                ...
                quantity: item.duration ? parseInt(item.duration) * 30 : 30, // Default quantity
            ...
            medicationsHtml += `
                <div id="${medicineId}" class="flex justify-between items-center p-2 border-b border-gray-100">
                    <div>
                        <h5 class="text-sm font-medium text-ayur-brown-800">${item.name}</h5>
                        <p class="text-xs text-ayur-brown-600">${item.dosage}, ${item.frequency}</p>
                        <p class="text-xs text-ayur-brown-500">${item.duration} • ${item.instructions || 'No special instructions'}</p>
                    </div>
                    <div class="flex items-center">
                        <span class="text-xs text-ayur-brown-700 mr-2">Qty: <input type="number" class="w-16 px-2 py-1 border border-gray-300 rounded text-xs" value="${selectedMedicines[index].quantity}" min="1" onchange="updateMedicineQuantity('${medicineId}', this.value)"></span>
                        <input type="checkbox" class="form-checkbox text-ayur-green-600 h-4 w-4" checked onchange="toggleMedicineSelection('${medicineId}', this.checked)">
                    </div>
                </div>
            `;
        });
        ...
        if (patientCards) {
            patientCards.innerHTML = `
                <div class="flex items-start">
                    <img class="h-12 w-12 rounded-full" src="https://storage.googleapis.com/uxpilot-auth.appspot.com/avatars/avatar-5.jpg" alt="Patient avatar">
                    <div class="ml-3">
    ...
    // Test bill summary functionality
    // window.testBillSummary = function () {
    //     console.log('Testing bill summary functionality...');

    //     // Simulate patient selection
    //     const testPatientId = 1;
    //     const selectedPatientIdInput = document.getElementById('selectedPatientId');
    //     if (selectedPatientIdInput) {
    //         selectedPatientIdInput.value = testPatientId;
    //         console.log('Test patient ID set:', testPatientId);
    //     }

    //     // Simulate prescription selection
    //     const testPrescriptionId = 1;
    //     const selectedPrescriptionIdInput = document.getElementById('selectedPrescriptionId');
    //     if (selectedPrescriptionIdInput) {
    //         selectedPrescriptionIdInput.value = testPrescriptionId;
    //         console.log('Test prescription ID set:', testPrescriptionId);
    //     }

    //     // Add a test additional medicine
    //     const testMedicine = {
    //         id: 1,
    //         name: 'Test Medicine',
    //         code: 'TEST001',
    ...
    //     additionalMedicines.push(testMedicine);
    //     updateAdditionalMedicinesDisplay();
    //     updateBillSummary();

    //     // Also add a test prescription medicine
    //     const testPrescriptionMedicine = {
    //         id: 'medication_test_1',
    //         name: 'Prescription Test Medicine',
    //         dosage: '500mg',
    ...
    //     selectedMedicines.push(testPrescriptionMedicine);
    //     updateBillSummary();

    //     console.log('Test medicines added to bill summary');
    // };
